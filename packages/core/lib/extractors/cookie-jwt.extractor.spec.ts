...
describe("CookieJwtExtractor", () => {
  describe("with default cookie name", () => {
    ...
    describe("hasAuthenticationInfo", () => {
      it.each([
        [
          "HTTP request has access_token cookie",
          { cookies: { access_token: "token123" } },
        ],
        [
          "request with cookies object",
          { cookies: { access_token: "token456", other: "value" } },
        ],
      ...
      it.each([
        ...
        [
          "access_token cookie is whitespace",
    ...
    describe("getAuthorizationToken", () => {
      it.each([
        [
          "HTTP request access_token cookie",
          { cookies: { access_token: "token123" } },
          "token123",
        ],
        [
        ...
        [
          "cookie with whitespace",
          { cookies: { access_token: "  token789  " } },
          "token789",
        ],
  ...
  describe("with custom cookie name", () => {
    ...
    it.each([
      ...
      if (method === "hasAuthenticationInfo") {
        ...
        expect(extractor.getAuthorizationToken(requestWithCustom)).toBe(
          "token123",
        );
