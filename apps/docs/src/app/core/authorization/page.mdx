export const metadata = {
  ...
  description: 'Restrict routes based on Cognito user groups or OAuth scopes.',
}
...
Machine-to-machine clients (client credentials flow) have no user and no groups. Their access token carries OAuth scopes instead:
...
The token's `scope` claim must contain every listed scope. Use `allowedScopes` to accept any one of them instead, e.g. `['orders/read', 'orders/admin']`. Scopes are case-sensitive, unlike groups.
...
**requiredScopes**: The token must carry ALL of these scopes.

**allowedScopes**: The token must carry AT LEAST ONE of these scopes.
