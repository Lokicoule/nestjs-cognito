export const metadata = {
  title: '@nestjs-cognito/graphql API - NestJS Cognito',
  description: 'Technical reference for @nestjs-cognito/graphql package.'
}

# @nestjs-cognito/graphql

<Note>
**Deprecated since 4.0.** Use the [`@nestjs-cognito/auth`](/api/auth) decorators on resolvers. The `Gql*` exports below are aliases of them and will be removed in 5.0.
</Note>

GraphQL-specific decorators and guards for NestJS applications.

## Installation

```bash
npm install @nestjs-cognito/graphql @nestjs-cognito/auth @nestjs-cognito/core
```

## Decorators

### Guards

#### `@GqlAuthentication(options?: { tokenUse?: 'access' | 'id' })`
Apply JWT authentication to GraphQL resolvers or operations. `tokenUse` rejects the other token type.

```typescript
@Resolver()
@GqlAuthentication()
export class UserResolver {}
```

#### `@GqlAuthorization(options: string[] | AuthorizationOptions)`
Require Cognito groups or OAuth scopes for GraphQL operations. `AuthorizationOptions`: `allowedGroups`, `requiredGroups`, `prohibitedGroups`, `allowedScopes`, `requiredScopes`.

```typescript
@Mutation()
@GqlAuthorization(['admin'])
async adminMutation() {}
```

#### `@PublicRoute()`
Bypass authentication for specific GraphQL operations. Import it from `@nestjs-cognito/auth`; there is no GraphQL-specific variant.

```typescript
@Query()
@PublicRoute()
async publicQuery() {}
```

### Parameter Decorators

#### `@GqlCognitoUser(property?: string)`
Extract authenticated user from GraphQL context.

```typescript
@Query()
async me(@GqlCognitoUser() user: CognitoJwtPayload) {}
```

#### `@GqlCognitoIdUser(property?: string)`
Extract ID token payload (validates token type).

```typescript
@Query()
async profile(@GqlCognitoIdUser() user: CognitoIdTokenPayload) {}
```

#### `@GqlCognitoAccessUser(property?: string)`
Extract access token payload (validates token type).

```typescript
@Query()
async permissions(@GqlCognitoAccessUser() token: CognitoAccessTokenPayload) {}
```

## Exports

**Decorators:**
- `@GqlAuthentication(options?)`
- `@GqlAuthorization(options: string[] | AuthorizationOptions)`
- `@PublicRoute()` (from `@nestjs-cognito/auth`)
- `@GqlCognitoUser(property?: string)`
- `@GqlCognitoIdUser(property?: string)`
- `@GqlCognitoAccessUser(property?: string)`

**Guards:**
- `GqlCognitoAuthGuard`
- `GqlCognitoGroupGuard`

**Constants:**
- `GQL_PUBLIC_ROUTE_METADATA`

## Guides

- [GraphQL](/integrations/graphql) - Complete GraphQL integration guide
