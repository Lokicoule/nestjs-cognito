...
**Problem:** You're using `@CognitoIdUser()` but sending an access token. The request is rejected with a 401.
...
**Cause:** The JWKS keys could not be fetched at startup. `CognitoAuthModule` preloads them when the app starts; if that fails, it logs `Could not preload the JWKS` and fetches them on the first request instead.

**Fix:** Check that the app can reach `https://cognito-idp.<region>.amazonaws.com` at startup.
...
```typescript
export class CustomExtractor implements CognitoJwtExtractor<Request> {
  hasAuthenticationInfo(request: Request): boolean {
    console.log('Request headers:', request.headers);  // ← Debug
    return Boolean(request.header('x-custom-token'));
  }

  getAuthorizationToken(request: Request): string | null {
    const token = request.header('x-custom-token');
...
**Problem:** The app fails at startup with `No JWT verifier configured: set jwtVerifier or jwtRsaVerifier`.

**Cause:** `CognitoAuthModule` received neither `jwtVerifier` nor `jwtRsaVerifier`. With `registerAsync`, check that the factory returns them.
...
1. Invalid configuration values (User Pool ID, Client ID)
