...
If your app client has a secret, pass it as the third argument. A function is read on every login, so a rotated secret applies without restarting:

```typescript
CognitoTestingModule.register(
  { identityProvider: { region: process.env.COGNITO_REGION } },
  undefined,
...
### Machine-to-machine and expired tokens

`CognitoMockService` also issues client credentials tokens, and `expiresIn` lets you test expiry:

```typescript
const mock = moduleRef.get(CognitoMockService);

// Access token for a machine-to-machine client
const m2mToken = mock.createClientCredentialsToken('billing-service', ['orders/read']);

// Every token issued from now on is already expired
mock.setMockConfig({ enabled: true, user: { username: 'john' }, expiresIn: -60 });
```
