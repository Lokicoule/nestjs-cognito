...
`credentials` also accepts a credential provider. The AWS SDK caches its result and calls it again when the credentials expire, so they are renewed without restarting the app:
...
CognitoModule.register({
  identityProvider: {
    ...
    credentials: fromTemporaryCredentials({
      params: { RoleArn: process.env.COGNITO_ADMIN_ROLE_ARN },
    }),
  },
});
```
